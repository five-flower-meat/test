      ...
      success: function (res = {}) {
        if (res.return_code === '-1') {
          ...
          if (res.data && res.data.returnUrl) {
            utils.Toast('登录赢好礼，您可以用咪咕系APP的任一账号进行登录，也可用短信方式登录');
            setTimeout(function () {
...
export default {
  loginJudge: API('/migu/miguDay/loginJudge.action'),
  queryRemainingTimes: API('/migu/miguDay/queryRemainingTimes.action'),
  winDrawChance: API('/migu/miguDay/winDrawChance.action'),
  analyzeRedirect: API('/migu/miguDay/redirect.action'),
  luckyDraw: API('/migu/miguDay/luckyDraw.action'),
};